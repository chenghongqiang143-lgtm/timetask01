  ...
  const StatsModal = () => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-stone-900/60 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-lg flex flex-col border border-stone-300 shadow-2xl overflow-hidden max-h-[85vh]">
                <div className="px-5 py-4 bg-stone-50 border-b border-stone-200 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-2.5">
                        <TrendingUp size={18} className="text-primary" />
                        <div>
                            <h3 className="font-black text-stone-800 text-sm">评估趋势</h3>
                ...
                <div className="p-5 overflow-x-auto custom-scrollbar flex-1 bg-white">
                    <div className="min-w-full">
                            ...
                            {ratingItems.map(item => (
                                <div key={item.id} className="flex items-center group">
                                    <div className="w-16 sm:w-24 shrink-0 pr-2">
                                        <div className="text-[11px] font-bold text-stone-500 truncate group-hover:text-stone-900 transition-colors" title={item.name}>
                                            {item.name}
                                        </div>
                                    </div>
                                    {weekDates.map(d => {
                                        ...
                                        return (
                                            <div key={k} className="flex-1 h-9 flex items-center justify-center p-0.5">
                                                <div className={cn(
                                                    "w-full h-full rounded-lg shadow-sm transition-all flex items-center justify-center text-[10px] font-black",
                                                    getCellColor(score)
                                                )}>
                        ...
                        <div className="mt-8 pt-6 border-t border-stone-100 flex items-center justify-center gap-4 text-[10px] font-black text-stone-400 uppercase tracking-widest">
      ...
      return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-stone-900/60 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-lg flex flex-col border border-stone-300 shadow-2xl overflow-hidden max-h-[85vh]">
                ...
                <div className="p-4 overflow-y-auto custom-scrollbar flex-1 bg-stone-50/30">
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {shopItems.map(item => (
                            <div key={item.id} className="bg-white rounded-xl p-3 border border-stone-100 shadow-sm flex flex-col gap-1.5 transition-all group relative">
                                <button onClick={() => setEditingShopItem(item)} className="absolute top-1.5 right-1.5 p-1 text-stone-300 hover:text-stone-800 opacity-0 group-hover:opacity-100 transition-all bg-stone-50 rounded-md">
                                    <Edit2 size={10} />
                                </button>
                                <div className="text-2xl mt-0.5">{item.icon}</div>
                                <div className="text-left px-0.5 flex-1 min-h-[34px]">
                                    <h4 className="font-bold text-stone-800 text-[11px] truncate leading-tight">{item.name}</h4>
                                    <div className="text-[10px] font-black text-amber-500 mt-0.5 flex items-center gap-1">
                                        <Coins size={10} /> {item.cost}
                                    </div>
                                </div>
                                <button onClick={() => onRedeem(item)} disabled={!canAfford(item.cost)} className={cn("w-full py-1.5 rounded-lg text-[10px] font-black transition-all active:scale-95", canAfford(item.cost) ? "bg-stone-900 text-white shadow-sm" : "bg-stone-100 text-stone-300 cursor-not-allowed")}>
                                    {canAfford(item.cost) ? '兑换' : '不足'}
                                </button>
                            </div>
                        ))}
                         <button className="bg-white/40 rounded-xl p-2 border border-dashed border-stone-200 flex flex-col items-center justify-center gap-1 text-stone-400 hover:text-primary hover:border-primary transition-all min-h-[100px]" onClick={() => setEditingShopItem({ id: generateId(), name: '', cost: 10, icon: '🎁' })}>
                             <Plus size={16} />
                             <span className="text-[10px] font-bold">新增项目</span>
                         </button>
                    </div>
                </div>
                <div className="px-5 py-3 bg-white border-t border-stone-100 shrink-0">
                    <h4 className="text-[9px] font-black text-stone-400 uppercase tracking-widest mb-2">近期消费记录</h4>
                    <div className="flex gap-2 overflow-x-auto pb-1 custom-scrollbar">
                        {redemptions.slice(0, 5).map(r => (
                            <div key={r.id} className="px-2.5 py-1 bg-stone-50 rounded-lg border border-stone-100 flex items-center gap-1.5 shrink-0">
                                <span className="text-xs">{shopItems.find(s => s.id === r.shopItemId)?.icon || '🎁'}</span>
                                <span className="text-[10px] font-bold text-stone-500 truncate">{r.itemName}</span>
                                <span className="text-[10px] font-black text-rose-400">-{r.cost}</span>
                            </div>
                        ))}
                        {redemptions.length === 0 && <span className="text-[10px] text-stone-300 italic">空空如也...</span>}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div className="h-full bg-white overflow-y-auto custom-scrollbar">
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3">
             {ratingItems.map(item => {
                 ...
                 return (
                    <div key={item.id} className="bg-stone-50/50 rounded-2xl p-4 border border-stone-100 space-y-3.5 transition-all hover:bg-stone-50">
                        <div className="flex justify-between items-center px-0.5">
                            <h3 className="text-[13px] font-black text-stone-700">{item.name}</h3>
                            {selectedScore !== undefined && (
                                <span className={cn("text-[10px] font-bold px-2.5 py-1 rounded-full bg-white border border-stone-100 shadow-sm", getScoreColor(selectedScore))}>
                        ...
                        <div className="flex gap-2">
                            {SCORES.map(score => (
                                <button key={score} onClick={() => handleScoreSelect(item.id, score)} className={cn("flex-1 h-9 rounded-xl border text-[11px] font-black transition-all flex items-center justify-center active:scale-90", getScoreBg(score, selectedScore === score))}>
                                    {score > 0 ? `+${score}` : score}
                                </button>
                            ))}
                        </div>
             ...
             {ratingItems.length === 0 && (
                 <div className="py-12 flex flex-col items-center justify-center text-stone-300 gap-2 border-2 border-dashed border-stone-100 rounded-2xl bg-stone-50/20">
                     <Info size={24} />
                     <p className="text-[11px] font-bold">暂无评估项目，请点击右上角配置</p>
                 </div>
             )}
          </div>
        </section>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-stone-900/60 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl w-full max-w-md max-h-[85vh] overflow-hidden flex flex-col border border-stone-300 shadow-2xl">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-3">
                    {ratingItems.map(item => (
                        <div key={item.id} className="flex items-center justify-between p-4 bg-stone-50 rounded-xl border border-stone-100 group">
                            <span className="font-bold text-stone-700 text-[13px]">{item.name}</span>
                            <div className="flex gap-2">
                                <button onClick={() => setEditingItem(item)} className="p-2 hover:bg-white hover:text-stone-900 rounded-lg border border-transparent hover:border-stone-200 transition-all text-stone-400">
                                    <Settings2 size={16} />
                                </button>
                                <button onClick={() => deleteRatingItem(item.id)} className="p-2 hover:bg-rose-50 hover:text-rose-500 rounded-lg border border-transparent hover:border-rose-100 transition-all text-stone-400">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        </div>
                    ))}
                    <button onClick={() => setEditingItem({ id: generateId(), name: '', reasons: { [-2]: '极差', [-1]: '较差', [0]: '一般', [1]: '较好', [2]: '极好' } })} className="w-full py-4 rounded-xl border-2 border-dashed border-stone-200 text-stone-400 hover:text-stone-900 hover:border-stone-400 transition-all flex items-center justify-center gap-2 font-bold text-xs">
                        <Plus size={18} /> 新增评估维度
                    </button>
                </div>
            </div>
      ...
      {editingShopItem && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-stone-900/80 p-4 backdrop-blur-md">
              <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden flex flex-col border border-stone-400 shadow-2xl">
                  <div className="px-6 py-5 bg-stone-50 border-b border-stone-200 flex justify-between items-center shrink-0">
                      <h3 className="font-black text-sm text-stone-800">奖励项目设定</h3>
                  ...
                  <div className="p-6 space-y-5 overflow-y-auto max-h-[70vh] custom-scrollbar">
                      <div className="flex gap-4">
                          ...
                          <div className="space-y-2 flex-1">
                             <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">奖项名称</label>
                             <input type="text" value={editingShopItem.name} onChange={e => setEditingShopItem({...editingShopItem, name: e.target.value})} className="w-full h-16 px-4 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:bg-white focus:border-stone-400 font-bold text-[13px]" placeholder="如：一顿大餐" />
                          </div>
                      </div>
                      <div className="space-y-2">
                          <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">所需积分</label>
                          <div className="relative">
                              <Coins className="absolute left-4 top-1/2 -translate-y-1/2 text-amber-500" size={20} />
                              <input type="number" value={editingShopItem.cost} onChange={e => setEditingShopItem({...editingShopItem, cost: parseInt(e.target.value) || 0})} className="w-full h-14 pl-12 pr-4 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:bg-white focus:border-stone-400 font-black text-xl" />
                          </div>
                      </div>
                  </div>
                  <div className="p-4 bg-stone-50 border-t border-stone-200 flex gap-2 shrink-0">
                      <button onClick={() => handleDeleteShopItem(editingShopItem.id)} className={cn("px-4 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 font-bold text-[11px]", deleteConfirmId === editingShopItem.id ? "bg-rose-600 text-white flex-1" : "bg-white border border-rose-100 text-rose-500 hover:bg-rose-50")}>
                        {deleteConfirmId === editingShopItem.id ? '确认删除' : <Trash2 size={18} />}
                      </button>
      ...
      {editingItem && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-stone-900/80 p-4">
              <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden flex flex-col border border-stone-400 shadow-2xl">
                  <div className="px-6 py-5 bg-stone-50 border-b border-stone-200 flex justify-between items-center shrink-0">
                      <h3 className="font-black text-sm text-stone-800">编辑维度设定</h3>
                      <button onClick={() => setEditingItem(null)} className="p-2 hover:bg-stone-200 rounded-full transition-colors text-stone-400"><X size={20} /></button>
                  </div>
                  <div className="p-6 space-y-6 overflow-y-auto max-h-[70vh] custom-scrollbar">
                      <div className="space-y-2">
                          <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">维度标题</label>
                          <input type="text" value={editingItem.name} onChange={e => setEditingItem({...editingItem, name: e.target.value})} className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:bg-white focus:border-stone-400 font-bold" placeholder="如：学习效率" />
                      </div>
                      <div className="space-y-3">
                          <label className="text-[10px] font-black text-stone-400 uppercase tracking-widest ml-1">各分值描述</label>
                          {SCORES.map(s => (
                              <div key={s} className="flex items-center gap-3">
                                  <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center text-[11px] font-black border shrink-0", s > 0 ? "bg-emerald-50 text-emerald-600 border-emerald-100" : (s < 0 ? "bg-rose-50 text-rose-600 border-rose-100" : "bg-stone-50 text-stone-400 border-stone-200"))}>
                                      {s > 0 ? `+${s}` : s}
                                  </div>
                                  <input type="text" value={editingItem.reasons[s]} onChange={e => setEditingItem({...editingItem, reasons: {...editingItem.reasons, [s]: e.target.value}})} className="flex-1 px-4 py-2.5 bg-stone-50 border border-stone-100 rounded-xl text-xs font-bold focus:outline-none focus:bg-white" />
                              </div>
                          ))}
                      </div>
                  </div>
